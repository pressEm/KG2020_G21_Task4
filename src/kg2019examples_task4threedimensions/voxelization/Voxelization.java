...
public class Voxelization {
    ...
    public Voxelization(IModel model, Scene scene, float a) {
...
//            Коэфффициенты уравнения плоскости
            ...
            if ((Math.abs(c) > EPS)) {
                if ((isPointInTriangle(v, line3D) && (v.getZ() * pointCross.getZ() >= 0)
                        && (Math.abs(v.getZ()) <= Math.abs(pointCross.getZ())))) {
                    count++;
                }
            }
        }
    ...
    private static double func(Vector3 v1, Vector3 v2, Vector3 v3) {
        return v3.getX() * (v2.getY() - v1.getY())
                + v3.getY() * (v1.getX() - v2.getX())
